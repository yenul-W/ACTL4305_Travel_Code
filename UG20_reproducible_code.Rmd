...
For GenZ, families, spring and cruises
...
For east asia and large groups
```{r sensitivity_analysis2, echo=FALSE, warning=FALSE, message=FALSE}
#  scenarios for east asia and large groups
scenarios <- list(
  list(name = "East Asia", filter = expr(east_asia == 1)),
  list(name = "Large Groups", filter = expr(number_travellers > 5)),
  list(name = "East Asia & Large Groups", filter = expr((east_asia == 1) & number_travellers > 5))
)
...
ggplot(sensitivity_results, aes(x = Scenario,
  ...
  scale_y_continuous(labels = scales::percent_format(scale = 1)) +
  labs(
    title = "Sensitivity Analysis: East Asia & Large Groups",
    x = "Target Group",
    y = "Conversion Rate Uplift Relative to Baseline",
    fill = "Increase in Group %"
  ) +
